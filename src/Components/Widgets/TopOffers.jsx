...
const TopOffers = ({ product, productOffers }) => {
    const {
        buyCost,
        storageMonth,
        fulfillment,
    ...
    return (
        <CustomCard label={'Top Offers'}>
            <div className='w-full '>
                <div className=" border border-accent customScroll rounded-lg ">
                        ...
                        <tbody>
                            {productOffers?.offers?.map((offer, index) => (
                                ...
                                >
                                    ...
                                    {(() => {
                                        const { profit, roi } = calculateOfferProfitAndROI(product, offer?.price, storageMonth, fulfillment, buyCost);
                                        return (
                                            <>
                                                <td className={`px-1 py-1.5 text-center border-r border-accent ${profit < MIN_PROFIT && 'text-red-400'}`}>
                                                    {formatNumberWithCommas(profit)}
                                                </td>
                                                <td className={`px-1 py-1.5 text-center ${roi < MIN_ROI && 'text-red-400'}`}>
